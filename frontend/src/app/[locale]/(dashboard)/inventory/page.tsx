  ...
  return (
    <div className="space-y-4 max-w-7xl">
        ...
        <div className="flex gap-2 flex-wrap">
          <Select value={riskLevel || "all"} onValueChange={(v) => { setRiskLevel(v === "all" ? "" : v); setPage(1); }}>
            ...
            <SelectContent>
              ...
              {(filters?.risk_levels ?? ["CRITICAL", "HIGH", "MEDIUM", "LOW"]).map((r) => (
      ...
      <div className="flex items-center justify-between text-sm text-slate-500">
        <span>
          {isLoading ? "Loading..." : `${total} batches`}
        </span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="text-left p-4 font-medium text-slate-600">{t("product")}</th>
                <th className="text-left p-4 font-medium text-slate-600 hidden md:table-cell">{t("batch")}</th>
                <th className="text-left p-4 font-medium text-slate-600">{t("expiry")}</th>
                <th className="text-right p-4 font-medium text-slate-600">{t("quantity")}</th>
                <th className="text-center p-4 font-medium text-slate-600">{t("risk")}</th>
                <th className="text-right p-4 font-medium text-slate-600">{t("loss")}</th>
                <th className="text-left p-4 font-medium text-slate-600 hidden lg:table-cell">Supplier</th>
              </tr>
            </thead>
                    ...
                    >
                      <td className="p-4">
                        <div>
                          ...
                          <p className="text-xs text-slate-400">{batch.category}</p>
                        </div>
                      </td>
                      <td className="p-4 hidden md:table-cell">
                        <span className="font-mono text-xs text-slate-600">{batch.batch_number}</span>
                      </td>
                      <td className="p-4">
                        <div>
                      ...
                      <td className="p-4 text-right">
                        <span className="font-medium text-slate-700">{batch.current_quantity}</span>
                      ...
                      <td className="p-4 text-right">
                        <span
                          className={`font-semibold ${
                            batch.estimated_loss > 0 ? "text-red-600" : "text-slate-400"
                          }`}
                        >
